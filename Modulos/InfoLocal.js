...
const db = SQLite.openDatabase("InfoUsuarioLocal.db");
...
export const borrarHoras = () => {
  db.transaction((tx) => {
    tx.executeSql(
      ...
      (_, result) => {
        console.log(
          "Todos los registros de la tabla Horas (local) han sido borrados."
        );
      },
      (_, error) => {
        console.log(
          "Error al borrar los registros de la tabla Horas (local): ",
          error
        );
